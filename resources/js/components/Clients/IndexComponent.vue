<template>
    <div class="p-3 text-center">
        <div id="" class="">
            <div class="m-19">
                <div class="">
                    <h1>Listado de Clientes</h1><br><br>
                </div>
                <div class="row col-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped table-bordered bg-white">
                            ...
                            <tbody>
                                <tr v-for="(item,index) in clients" :key="index" class="text-center">
                                    <td>{{item.id}}</td>
                    ...
                    <h3 class="bg-red" v-if="selectedclients.length == 0">NO SE ENCONTRARON REGISTROS!!!</h3>
                    ...
                    <div class="pagination col-12">
                        ...
                        <div class="text-right col-8">
                            <h5 class="text-info ml-5">{{selectedclients.length}} registros encontrados</h5>
                        </div>
                    </div>
                </div>
            </div>
    ...
    export default{
        ...
        created:function(){
            axios.get('/admin/clientes').then(res=>{
                ...
                this.totalPages=Math.ceil(this.selectedclients.length/this.pagination);
                this.searching(2);
            });
        ...
        methods:{
            ...
            updateTable(change){
                ...
                this.clients=this.selectedclients.slice(this.actualPage*this.pagination,this.actualPage*this.pagination+this.pagination);
            },
            updateTotalPages(){
                this.totalPages = Math.ceil(this.selectedclients.length/this.pagination);
            },
            deletecake(item,index){
                this.$confirm('Realmente desea eliminar el cliente','Alerta',{
                    ...
                    axios.delete(`/admin/clientes/${item.id}`).then(res=>{
                        ...
                        if(res.data['information'] === 'good'){
                            ...
                            this.searching(2);
                            this.showSuccessNotification('Cliente eliminado','El cliente fue eliminado exitosamente');
                        }else{
                            this.showErrorNotification('Error al eliminar','El cliente se encuentra asignado en un pedido');
                        }
            ...
            setAddView(){
                window.location.href="/admin/clientes/agregar";
            },
            setDetailsView(item){
                window.location.href="/admin/clientes/"+item.id;

            },
            setEditView(item){
                this.editing=true;
                window.location.href="/admin/clientes/editar/"+item.id;
            },
            sortTable(value){
                if(this.order === 1){
                    this.order=2;
                ...
                if(value ==='id'){
                    if(this.order===1){
                        this.selectedclients.sort(function(a,b){return a.id<b.id?-1:a.id>b.id});
                    }else{
                        this.selectedclients.sort(function(a,b){return a.id>b.id?-1:a.id<b.id});
                    }
                }else if(value === 'name'){
                    if(this.order===1){
                        this.selectedclients.sort(function(a,b){return a.name<b.name?-1:a.name>b.name});
                    }else{
                        this.selectedclients.sort(function(a,b){return a.name>b.name?-1:a.name<b.name});
                    }
                }else if(value === 'correo'){
                    this.selectedclients.sort(function(a,b){return a.email>b.email?-1:a.email<b.email});
                }
                this.updateTable(0);
            }
        }
