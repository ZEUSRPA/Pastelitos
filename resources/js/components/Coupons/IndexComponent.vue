<template>
    <div class="p-3 text-center">
        <div id="" class="">
            <div class="m-19">
                <div class="">
                    <h1>Listado de cupones</h1><br><br>
                </div>
                <div class="row col-12">
                        ...
                        <table class="table table-hover table-striped table-bordered bg-white">
                            <thead class="thead-dark">
                                <tr class="text-center">
                                    ...
                                    <th @click="sortTable('name')">PORCENTAJE DE DESCUENTO</th>
                                    <th @click="sortTable('price')">FECHA DE EXPIRACION</th>
                                    <th>OPCIONES</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(item,index) in coupons" :key="index" class="text-center">
                                    <td>{{item.id}}</td>
                                    <td class="w-50">{{item.percentage}}</td>
                                    <td class="w-25">{{item.expiration}}</td>
                                    <td class="m-0 p-0">
                    ...
                    <h3 class="bg-red" v-if="selectedcoupons.length == 0">NO SE ENCONTRARON REGISTROS!!!</h3>
                    ...
                    <div class="pagination col-12">
                        ...
                        <div class="text-right col-8">
                            <h5 class="text-info ml-5">{{selectedcoupons.length}} registros encontrados</h5>
                        </div>
                    </div>
                </div>
            </div>
    ...
    export default{
        ...
        created:function(){
            axios.get('/admin/cupones').then(res=>{
                ...
                // for(let x of this.allcoupons){
                //     x.expiration = x.expiration.toString();
                // }
                console.log(this.allcoupons);
                this.selectedcoupons=this.allcoupons;
                this.totalPages=Math.ceil(this.selectedcoupons.length/this.pagination);
                this.searching(2);
            });
        ...
        methods:{
            ...
            updateTable(change){
                ...
                this.coupons=this.selectedcoupons.slice(this.actualPage*this.pagination,this.actualPage*this.pagination+this.pagination);
            },
            updateTotalPages(){
                this.totalPages = Math.ceil(this.selectedcoupons.length/this.pagination);
            },
            deletecake(item,index){
                this.$confirm('Realmente desea eliminar el cupon','Alerta',{
                    ...
                    axios.delete(`/cupones/${item.id}`).then(res=>{
                        if(res.data['information'] === 'good'){
                            ...
                            this.searching(2);
                            this.showSuccessNotification('Cupon eliminado','El cupon fue eliminado exitosamente');
                        }else{
                            this.showErrorNotification('Error al eliminar','El cupon se encuentra asignado en un pedido');
                        }
            ...
            setAddView(){
                window.location.href="/admin/cupones/agregar";
            },
            setDetailsView(item){
                window.location.href="/admin/cupones/"+item.id;

            },
            setEditView(item){
                this.editing=true;
                window.location.href="/admin/cupones/editar/"+item.id;
            },
            sortTable(value){
                if(this.order === 1){
                    this.order=2;
                ...
                if(value ==='id'){
                    if(this.order===1){
                        this.selectedcoupons.sort(function(a,b){return a.id<b.id?-1:a.id>b.id});
                    }else{
                        this.selectedcoupons.sort(function(a,b){return a.id>b.id?-1:a.id<b.id});
                    }
                }else if(value === 'name'){
                    if(this.order===1){
                        this.selectedcoupons.sort(function(a,b){return a.name<b.name?-1:a.name>b.name});
                    }else{
                        this.selectedcoupons.sort(function(a,b){return a.name>b.name?-1:a.name<b.name});
                    }
                }else if(value === 'price'){
                    this.selectedcoupons.sort(function(a,b){return a.price>b.price?-1:a.price<b.price});
                }
                this.updateTable(0);
            }
        }
