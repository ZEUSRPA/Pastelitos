<template>
    <div class="p-3 text-center">
        <div id="" class="">
            <div class="m-19">
                <div class="">
                    <h1>Listado de pasteles</h1><br><br>
                </div>
                <div class="row col-12">
                    ...
                    <h3 class="bg-red" v-if="selectedcakes.length == 0">NO SE ENCONTRARON REGISTROS!!!</h3>
                    ...
                    <div class="pagination col-12">
                        ...
                        <div class="text-right col-8">
                            <h5 class="text-info ml-5">{{selectedcakes.length}} registros encontrados</h5>
                        </div>
                    </div>
                </div>
            </div>
    ...
    export default{
        ...
        created:function(){
            axios.get('/pasteles').then(res=>{
                ...
                this.totalPages=Math.ceil(this.selectedcakes.length/this.pagination);
                this.searching(2);
            });
        ...
        methods:{
            ...
            updateTable(change){
                ...
                this.cakes=this.selectedcakes.slice(this.actualPage*this.pagination,this.actualPage*this.pagination+this.pagination);
            },
            updateTotalPages(){
                this.totalPages = Math.ceil(this.selectedcakes.length/this.pagination);
            },
            ...
            sortTable(value){
                ...
                if(value ==='id'){
                    if(this.order===1){
                        this.selectedcakes.sort(function(a,b){return a.id<b.id?-1:a.id>b.id});
                    }else{
                        this.selectedcakes.sort(function(a,b){return a.id>b.id?-1:a.id<b.id});
                    }
                }else if(value === 'name'){
                    if(this.order===1){
                        this.selectedcakes.sort(function(a,b){return a.name<b.name?-1:a.name>b.name});
                    }else{
                        this.selectedcakes.sort(function(a,b){return a.name>b.name?-1:a.name<b.name});
                    }
                }else if(value === 'price'){
                    this.selectedcakes.sort(function(a,b){return a.price>b.price?-1:a.price<b.price});
                }
                this.updateTable(0);
            }
        }
