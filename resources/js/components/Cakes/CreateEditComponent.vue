                    ...
                    <div class="col-12">
                        <dir class="row justify-content-left m-0 p-0">
                            <div class="col-xs-12 col-sm-6 col-xl-4 m-0 p-0 text-left">
                                ...
                                <h3>Nombre</h3>
                                <div class="w-100">
                                    <input type="text" placeholder="Nombre" class="form-control mb-2 text-left" v-model="cake.name">
                                </div>
                                <br>
                                <h3>Descripcion</h3>
                                <div class="w-100">
                                    <input type="text" placeholder="Descripcion" class="form-control mb-2 text-left" v-model="cake.description">
                                </div>
                                <br>
                                <h3>Precio</h3>
                                <div class="w-100">
                                       <input type="number" step="0.1" placeholder="Precio" class="form-control text-center" v-model="cake.price">
                                </div>
                                
                                <div class="row col-12">
                                    <div class="col-sm-12 text-center" v-if="selectedView === 'addView'">
                                        <button class="btn btn-success form-button m-3 col-sm-12 col-lg-6" @click="addProcedure()"><i class="el-icon-finished w-100 " ></i></button>
                                    </div>
                                    <div class="col-sm-12 text-center" v-if="selectedView === 'updateView'">
                                        <button class="btn btn-success form-button m-3 col-sm-12 col-lg-6" @click="updateProcedure()"><i class="el-icon-finished w-100 " ></i></button>
                                    </div>
                                </div>
                            </div>
                        </dir>
        ...
        methods:{
            setParams(){
                ...
                if (document.getElementById("deleteImg").style.visibility === "visible") {
                    ...
                    var file2 = new Blob([new Uint8Array(this.array)], { type: "image/png" });
            ...
            addProcedure(){
                ...
                axios.post('/pasteles',params).then(res=>{
                    ...
                    this.showSuccessNotification('Pastel agregado',"Pastel agregado exitosamente.");
                    this.resetFields();
            ...
            updateProcedure(){
                ...
                axios.post('/pasteles/',params).then(res=>{
                    const index=this.allcakes.findIndex(search=>search.id === res.data.id);
                    this.allcakes[index]=res.data;
                    this.showSuccessNotification('Pastel editado','Se ha editado el pastel exitosamente');
                    window.location.href="/pasteles/";
                });
